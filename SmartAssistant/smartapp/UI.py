...
def run():
    ...
    if db_path and not st.session_state.workflow_generated:
        try:
            st.session_state.app = create_workflow(chatbot_graph)
            st.session_state.sqlapp = create_workflow(schema=db_schema, session=SessionLocal)            
            image_path = "graph_image.png"
