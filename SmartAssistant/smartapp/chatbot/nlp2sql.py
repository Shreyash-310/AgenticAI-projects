...
def create_workflow(db_path):
    ...
    workflow.add_node("check_relevance", lambda state, config: check_relevance(state, config, db_schema))
    workflow.add_node("convert_to_sql", lambda state, config: convert_nl_to_sql(state, config, db_schema))
    # workflow.add_node("execute_sql", execute_sql)
    workflow.add_node("execute_sql", lambda state: execute_sql(state, SessionLocal))
...
if __name__ == '__main__':
    ...
    db_path = "D:/GenAI-Practice/AgenticAI-Projects/Text2SQL/data/sql/bank_domain.db"
    app = create_workflow(db_path)
